...
class PolykeyClient {
  ...
  static async AddPeerB64(peerInfoB64: string): Promise<boolean> {
    const res = pb.BooleanMessage.deserializeBinary(await ipcRenderer.invoke('AddPeerB64', peerInfoB64))
    return res.getB()
  }
  ...
  static async GetKey(keyName: string): Promise<string> {
    const encodedRequest = new pb.StringMessage()
    encodedRequest.setS(keyName)
    const res = pb.StringMessage.deserializeBinary(await ipcRenderer.invoke('GetKey', encodedRequest.serializeBinary()))
    return res.getS()
  }

  static async GetLocalPeerInfo(): Promise<pb.PeerInfoMessage.AsObject> {
    const res = pb.PeerInfoMessage.deserializeBinary(await ipcRenderer.invoke('GetLocalPeerInfo'))
    return res.toObject()
  }

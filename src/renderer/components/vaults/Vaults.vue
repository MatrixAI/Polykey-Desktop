      ...
      <v-col class="main-panel fill-parent-height">
        <v-card>
          ...
          <v-list v-else-if="secretNames.length != 0">
            <v-list-item v-for="item in secretNames" :key="item" link :ripple="false" @click="selectSecret(item)">
              {{item}}
              <v-spacer></v-spacer>
              <v-btn link icon small color="info" @click="newSecret()">
                <v-icon>fas fa-edit</v-icon>
              </v-btn>
              <v-btn link icon small color="warning" @click="deleteSecret()">
                <v-icon>fas fa-trash</v-icon>
              </v-btn>
            </v-list-item>
          </v-list>
...
export default class Vaults extends Vue {
  ...
  public get pathList() {
    const list = [
      { type: "vault", name: this.selectedVaultName },
    ];
